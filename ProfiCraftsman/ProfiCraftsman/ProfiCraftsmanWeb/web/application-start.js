define(['kendo/kendo.core'], function () {
    ...
    require(['router', 't!l!master', 'master-footer', 'jqueryui'], function (Router, MasterView, MasterFooterView) {
        ...
        masterView.$el.prepend('<div style="height: 40px; position: relative;"></div>');
        masterView.$el.append('<div style="height: 40px; position: relative;"></div>');
        masterView.$el.find('#wrapperbody').css('min-height', Math.max(600, $(window).outerHeight() - 100));
        ...
        if (!Backbone.history.fragment)
            router.navigate('Orders', { replace: true, trigger: true });
